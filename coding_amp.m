function EncodedMessage = coding_amp(message, bitrate)
    load("Mapset.mat", "Mapset");
    binaryMessage = [];
    messageLength = strlength(message);
    n = (strlength(message) * 5) / bitrate;
    fs = 100;
    
    % Construct binary message from Mapset
    ...
    flag = 1;
    for k = 1 : bitrate : stringLength
        index1 = k;
        index2 = min(k + bitrate - 1, stringLength);
        out{flag} = binaryMessage(index1: index2);
        flag = flag + 1;
    end
    ...
    for k = 1:n
        title(["Bit-Rate = ", int2str(bitrate)]);
